...
class DataBase:
    ...
    def Ajout_plante(self, token, dico):
        ...
        if self.is_connect:
            self.conn = connect_db()
            self.cursor = self.conn.cursor()
            self.cursor.execute("INSERT INTO Plant (plantDescription,plantAdress, name, timecount, UserId, species) VALUES (?,?,?,?,?,?)",(self.dico["plantDescription"],self.dico["plantAdress"], self.dico["name"], md5_hash(self.dico["duree_garde"]),self.is_connect,"species"))
            self.conn.commit()
            self.insert = self.cursor.fetchone()
            return True
        else:
